...
description: "Entry point to the Messages repo: what it is (a default SMS/MMS app that also embeds an HTTP gateway and webhook engine), the top-level layout, build/test commands, and a task-routing map into the wiki hierarchy."
...
and an outbound-only GMweb long-poll bridge. Turn any Android phone into a SMS/MMS
automation endpoint with no third-party cloud fees. The gateway hardens itself against
abuse: per-IP brute-force lockout (8 failed auth attempts in 10 minutes → 5-minute
lockout) and a 1 MB request-body cap, on top of the `X-API-Key`. It is a heavily extended
fork of
[manjitrana/Messages](https://github.com/manjitrana/Messages)
(current version `2.6.19`, versionCode `61`; `compileSdk` 36, `minSdk` 26 / Android 8.0+).

One invariant governs the whole data plane: the **system Telephony content providers are
the durable source of truth**. The Room database (`MessagesDatabase`) is a local
read-shadow the UI reads from, the gateway's inbox reads come from the providers, incoming
dispatch consumes the persisted provider row (its real id + threadId) rather than
broadcast extras, and no component may claim authority over them. See
...
│   │       ├── data/             #   Room shadow, DAOs, FTS, TelephonySyncCoordinator
│   │       ├── gateway/          #   GatewayService, ConnectionSupervisor, GatewayServer,
│   │       │                     #   WebhookEngine, Heartbeat/Registration, OutboxPoller
│   │       ├── receiver/         #   SmsReceiver, MmsReceiver, IncomingMessageDispatcher, boot
│   │       ├── sms/ mms/ eve/    #   send pipeline, scheduled sends, EveSmsQueue
...
│                                 #   architecture-v2-sync.md, migration strategy
...
- Unit tests run on a bare JVM because `testOptions { unitTests.isReturnDefaultValues = true }`
  makes Android APIs like `android.util.Log` no-op. The instrumented suite is a placeholder;
...
| Sending: SIM/SMSC prefs, rate limiting, SENT/DELIVERED status, MMS, scheduled sends | [Outgoing Send Pipeline](/openwiki/architecture/outgoing-messaging.md) |
...
| GMweb long-poll bridge, wake lock, outbox polling | [GMweb Pull Bridge](/openwiki/integrations/gmweb-pull.md) |
